...
module.exports = async function (options) {
  try {
    const {database, user, password, host, port, schema, pattern, pureRoot} = options
    ...
    const purescriptFiles = files
      ...
      .map((file, i) => {
        const parsed = path.parse(filenames[i])
        return getPurescriptCode(pureRoot, `${parsed.dir}/${parsed.name}`, tables, file)
      })
