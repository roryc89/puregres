...
const getPurescriptCode = (filename, tables, sqlCode) => {
  ...
  const getRegularArg = (fromClause) => R.pipe(
    R.filter(hasAParam),
    R.map(getWhereArgNameAndType(tables, fromClause))
  )

  const getArgs = (fromClause) => R.pipe(
    (whereClause) => {
      return whereClause.A_Expr
      ? [whereClause]
    ...
    R.chain((arg) => arg.A_Expr.rexpr && arg.A_Expr.rexpr.SubLink
      ...
      : getRegularArg(fromClause)([arg])
    ),
    R.sortBy(R.prop('number'))
  )

  const args = getArgs(fromClause)(whereClause)
